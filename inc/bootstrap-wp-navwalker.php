...
class understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<ul class=\" site-nav__mega-menu mega-menu\" role=\"menu\">\n";
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			$classes[]   = 'site-nav__item nav-item menu-item-' . $item->ID;
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '<div class="menu-toggle"><button aria-label="Menu" class="hamburger"><i aria-controls="navigation" aria-expanded="false" role="img" class="fa fa-bars"></i><i aria-controls="navigation" aria-expanded="false" role="img" class="fa fa-times"></i></button></div><ul';
			if ( $menu_class ) {
			...
			$fb_output .= '</ul>';
			if ( $container ) {
				$fb_output .= '</' . $container . '><strong>hello</strong>';
			}
			echo $fb_output;
		}
	}
